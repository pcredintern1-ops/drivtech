...
export default function AboutSection() {
  ...
  return (
    <section id="about"
      ...
      style={{ background: 'linear-gradient(160deg, #ecedef 0%, #f0f1f3 45%, #eaecee 100%)' }}>
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-8 lg:px-12">
        ...
        <div className="grid lg:grid-cols-[52%_48%] gap-6 xl:gap-8 items-center">
          ...
          <div ref={arrowRef}
            ...
            <motion.div
              ...
              style={{ width: '115%', maxWidth: '115%' }}>
              <img
                src="/about-arrow-new.png"
        ...
        <motion.div
          ...
          className="flex flex-col sm:flex-row sm:items-stretch mt-6 md:mt-8 max-w-2xl">
          {features.map(({ icon: Icon, title, desc }, i) => (
            <>
              {i > 0 && <div key={`sep-${i}`} className="hidden sm:block w-px bg-gray-300/60 shrink-0 self-stretch"/>}
              {i > 0 && <div key={`sep-mob-${i}`} className="sm:hidden h-px bg-gray-300/60 w-full my-1"/>}
              <motion.div key={title} variants={fadeFromLeft}
                className="flex flex-row items-center gap-4 py-5 sm:py-6 flex-1 pl-4 pr-6 sm:pl-6 sm:pr-8 rounded-3xl cursor-default"
                ...
                <motion.div
                  ...
                  <Icon size={18} className="text-[#65a30d]" strokeWidth={1.6}/>
                </motion.div>
                <div>
                  <h4 className="font-heading font-bold text-gray-900 text-sm mb-1 leading-snug whitespace-nowrap">{title}</h4>
                  <p className="text-gray-400 text-xs leading-relaxed max-w-[110px]">{desc}</p>
                </div>
              </motion.div>
            </>
          ))}
